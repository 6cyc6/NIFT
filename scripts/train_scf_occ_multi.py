...
model = vnn_occupancy_scf_net.VNNOccScfNetMulti(latent_dim=256, o_dim=opt.o_dim, sigmoid=True).cuda()
...
root_path = os.path.join(opt.logging_root, opt.experiment_name)
loss_fn = val_loss_fn = losses.occ_sdf_net
...
training.train_scf_occ(model=model_parallel, train_dataloader=train_dataloader, val_dataloader=val_dataloader, epochs=opt.num_epochs,
                       ...
                       clip_grad=False, val_loss_fn=val_loss_fn, mtl=opt.mtl, lw=opt.lw, overwrite=True)
