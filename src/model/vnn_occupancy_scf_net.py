...
class VNNOccScfNet(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.return_features:
            out_dict['occ'], out_dict['scf'], out_dict['features'] = self.decoder(query_points, z)
        else:
            out_dict['occ'], out_dict['scf'] = self.decoder(query_points, z)
    ...
    def forward_latent(self, z, coords, return_dict=False):
        ...
        out_dict['occ'], out_dict['scf'], out_dict['features'] = self.decoder(coords, z)
...
class DecoderInnerMultiHead(nn.Module):
    ''' Decoder class.
    ...
    def __init__(self, dim=3, z_dim=128, c_dim=128, o_dim=1,
        ...
        self.fc_out_scf = nn.Linear(hidden_size, 5)
    ...
    def forward(self, p, z, c=None, **kwargs):
        ...
        out_occ = self.fc_out_occ(self.actvn(net))
        out_scf = self.fc_out_scf(self.actvn(net))
